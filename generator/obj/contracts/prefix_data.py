...
class PrefixData:
    ...
    def operator_mobile_codes(self, value: Optional[List[str]]) -> Optional[List[str]]:
        if (value is None):
            return self.__dict["OPERATOR_CODES"]["MOBILE"]
        self.__dict["OPERATOR_CODES"]["MOBILE"] = value

    def operator_desk_codes(self, value: Optional[List[str]]) -> Optional[List[str]]:
        if (value is None):
            return self.__dict["OPERATOR_CODES"]["DESK"]
        self.__dict["OPERATOR_CODES"]["DESK"] = value


    def start_with_desk(self, value: Optional[bool]) -> Optional[bool]:
        if (value is None):
